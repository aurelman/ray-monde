 ...
 * @author Aurélien
 */
public class XMLSurfaceBuilder {
    
    /**
     * The width attribute name.
     */
    ...
    /**
     * Builds a <code>Surface</code> object from the specified XML element node.
     *
     * @param element The XML element which describes the surface to build.
     * @return The resulting surface.
     * 
     * @throws ParsingException If the current element does not provide any
     * position element or dimension node.
     */
    public Surface build(final Element element) throws ParsingException {
        
        // Position
        String eltName = XMLSurfaceBuilder.POSITION_ELEMENT_NAME;
        Element elt = (Element)element.getElementsByTagName(eltName).item(0);
        
        if (elt == null) {
            throw new ParsingException(
                String.format(XMLSurfaceBuilder.NODE_MISSING_MESSAGE,
                    eltName));
        }
        ...
        double x, y, z;
        try {
            attributeName = XMLSurfaceBuilder.X_ATTRIBUTE_NAME;
            x = Double.parseDouble(elt.getAttribute(attributeName));
            attributeName = XMLSurfaceBuilder.Y_ATTRIBUTE_NAME;
            y = Double.parseDouble(elt.getAttribute(attributeName));
            attributeName = XMLSurfaceBuilder.Z_ATTRIBUTE_NAME;
            z = Double.parseDouble(elt.getAttribute(attributeName));
        } catch (NumberFormatException ex) {
            throw new ParsingException(
                String.format(
                    XMLSurfaceBuilder.MALFORMED_ATTRIBUTE_MESSAGE,
                    attributeName,
                    eltName));
        }
        ...
        // Dimension
        eltName = XMLSurfaceBuilder.DIMENSION_ELEMENT_NAME;
        ...
        try {
            attributeName = XMLSurfaceBuilder.WIDTH_ATTRIBUTE_NAME;
            width = Integer.parseInt(elt.getAttribute(attributeName));

            attributeName = XMLSurfaceBuilder.HEIGHT_ATTRIBUTE_NAME;
            height = Integer.parseInt(elt.getAttribute(attributeName));
        } catch (NumberFormatException ex) {
            throw new ParsingException(
                String.format(
                    XMLSurfaceBuilder.MALFORMED_ATTRIBUTE_MESSAGE,
        ...
        return new Surface(position, width, height);
    }
}
